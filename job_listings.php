...
include_once "functions.php";
    ...
    //add a new item to DB
            print "<h3>Current Job Positions</h3>
            ...
            foreach($rows as $row)
            {
                ...
                {
                    print "<li id='company$id'>Works at <a href='php_company.php?id=$companyid'>".$from['company']. "</a> with the job title <u>$title</u> <a href='#' class='remove_company' id='$companyid'><b>(REMOVE)</b></a>";
                    if (sizeof($game_links) > 0)
                    {
                        print "<ul> ";
                        for ($i = 0; $i < sizeof($game_links); $i++)
                             {
                                print "<li id='game{$game_links[$i][0]}'>".$game_links[$i][1]." <a href='#' class='remove_game' id='{$game_links[$i][0]}'><b>(REMOVE)</b></a></li>";
...
$(document).on("click",".remove_company",function(e){
    var company_id = this.id;
     ...
     $.ajax({
    		   	type: "GET",
			url: "process_remove_jobs.php?removejob=true",
			data: 'id=' + company_id,
        		success: function(msg){
                    
 	          		  $("#company"+company_id).html(msg)
 		        },
			error: function(){
				alert("failure");
				}
...
$(document).on("click",".remove_game",function(e){
    var game_id = this.id;
     ...
     $.ajax({
    		   	type: "GET",
			url: "process_remove_jobs.php?removegame=true",
			data: 'id=' + game_id,
        		success: function(msg){
                 
 	          		  $("#company"+game_id).html(msg)
 		        },
			error: function(){
				alert("failure");
				}
